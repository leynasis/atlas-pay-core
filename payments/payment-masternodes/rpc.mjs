...
export async function rpc(id, method, params = [], wallet, timeout = 15000) {
  const node = NODES[id];
  if (!node) throw new Error("Unknown payment masternode node");
...
export async function assertNode(id) {
  ...
  if (
    ...
    throw new Error(
      `Wrong chain on ${id}; refusing payment-masternode operation`,
    );
  for (const peer of peers) {
    if (
      !/^127\.0\.0\.1:\d+$/.test(peer.addr) ||
      (!peer.inbound &&
        !ALLOWED_P2P_PORTS.some((port) => peer.addr === `127.0.0.1:${port}`))
    )
