...
This is a local named-devnet experiment using real PSBTs and transactions from the selected Core profile: source-built LAVE Core by default, or official Dash 23.1.8 under `LAVEPAY_NETWORK=atlas`. The merchant wallet is on node 2 (`merchant`), and the customer wallet is on node 3 (`customer`). The payment workspace uses this signing policy in separate customer and merchant browser wallets; see [WALLET.md](WALLET.md). The customer CLI shares its durable journal with the customer browser wallet. The original server-funded regtest API remains a separate legacy experiment on port 4180.
...
- Requests pin the exact named chain, devnet name, ordinary genesis hash and named-devnet block 1 hash. LAVE requests additionally bind `network.currency`; existing Atlas request checksums stay unchanged. Testnet-style addresses and a PSBT alone do not identify a chain; ordinary genesis hashes are shared by some Dash test environments.
...
- The absolute fee limit is 0.0001 of the selected test currency (10000 base units), with an additional 10 base-unit/byte cap. A fixed 1 base-unit/byte construction rate is used in this lab. The final signed transaction must match the approved input/output template, and mempool acceptance is tested before broadcast.
...
The report is saved in `.runtime/lave/signer/integration-report.json` by default (the original `.runtime/signer/integration-report.json` for Atlas). Customer CLI state is stored in `.runtime/lave/signer/customer.sqlite` by default (Atlas keeps `.runtime/signer/customer.sqlite`); integration state uses a separate database. Neither report contains private keys or RPC credentials.
