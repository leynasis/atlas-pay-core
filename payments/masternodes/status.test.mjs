...
import { CHAIN, GENESIS_HASH, DEVNET_GENESIS_HASH, NODES } from "./config.mjs";
...
const valid = {
  ...
  capabilities: { instantSendVerified: true, chainLocksVerified: true },
};
...
test("unconfigured lab has no verified capability and never asks for RPC", async () => {
  const calls = [];
  const status = await getMasternodeStatus({
    now,
...
test("fresh pinned lab snapshot preserves actual proof booleans", async () => {
  const status = await getMasternodeStatus({ now, readSnapshot });
...
test("stale snapshots cannot advertise either verified capability", async () => {
  const status = await getMasternodeStatus({ now: now + 15001, readSnapshot });
  assert.equal(status.stale, true);
  assert.equal(status.capabilities.instantSendVerified, false);
  assert.equal(status.capabilities.chainLocksVerified, false);
});
test("future-dated snapshots fail closed", async () => {
  const status = await getMasternodeStatus({ now: now - 10000, readSnapshot });
  assert.equal(status.stale, true);
  assert.equal(status.capabilities.instantSendVerified, false);
});
...
test("all nine fixed ports and data directories are unique to the masternode lab", () => {
  ...
  assert.ok(
    Object.values(NODES).every((node) =>
      node.datadir.includes("/.runtime/masternodes/nodes/"),
    ),
  );
});
